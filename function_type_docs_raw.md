...
computable.  If it is, see [#Differentiable_functions].
...
TODO: fix links
 - Simulated Annealing (`ens::SA<>`)
 - CMAES (`ens::CMAES<>`)
 - CNE (`ens::CNE<>`)
 - Any optimizer for differentiable functions
 ...
 * L-BFGS (`ens::L_BFGS`)
 * FrankWolfe (`ens::FrankWolfe`)
 * GradientDescent (`ens::GradientDescent`)
 * CNE (`ens::CNE`)
 ...
 * CMAES (`ens::CMAES`)
 * any differentiable separable function optimizer # TODO: link
 * any (non-separable) arbitrary function optimizer
...
objective function, described in # TODO: link
...
The following optimizers can be used with differentiable functions:
 ...
 - `ens::MomentumSGD<>`
 - ...
 - any (non-separable) differentiable function optimizer # TODO: link

**Note**: using a non-separable differentiable function optimizer
will call `Evaluate(x, 0, NumFunctions() - 1)` and `Gradient(x, 0 g,
NumFunctions() - 1)`; if this is a very computationally intensive operation for
...
The following optimizers can be used in this way to optimize a categorical function:
 ...
 - ens::GridSearch (all parameters must be categorical)
...
solver.  The list of SDP solvers is below:
 ...
 - `ens::PrimalDualSolver`: primal-dual SDP solver
 - `ens::LRSDP`: low-rank accelerated SDP solver
